...
mysqli_query( $dbc, "SET time_zone='" . date( 'P' ) . "'" );
 ...
 @session_start();
     ...
     //get book details and its owner details
           $query2  = "SELECT * FROM book_stock WHERE book_id='".$_GET['id']."' LIMIT 1";
...
<head>
  ...
  <title>"<?php echo $row2['bname'];?>" | BookBoT, Stay connected with readers near you!</title>
  ...
  <link rel="stylesheet" href="css/viewbook.css" type="text/css" />                                     <!--for whole body--> 
...
<div id="main">
<div id="right_part">
  <div id="add_book_part">
    <h1>Do you have <br />this Book?</h1>
 <?php
        if(!isset($_SESSION['user_id'])){
?>
 <input type="button" value="ADD TO YOUR SHELF" id="login_for_sel_book" class="active_input" />
 <?php
        }else{ 
        //get user is selling this book
     $query6  = "SELECT * FROM user_books WHERE user_id='".$_SESSION['user_id']."' AND book_id='".$_GET['id']."'";
     $result6 = @mysqli_query($dbc, $query6)or die("Error of query6");
         if(mysqli_num_rows($result6)==0){
?>
<div id="success_add">
 <h4>Book added to bookshelf <br />successfully!</h4>
...
setInterval(hide_msg,10000);
       function hide_msg(){
     $('div#success_add').fadeOut();
};  
setInterval(hidefail_msg,10000);
       function hidefail_msg(){
     $('div#fail_add').fadeOut();
}; 
</script>
   <form method="POST" action="directaddbook.php" id="direct_add_book">
     <input type="button" value="ADD TO YOUR SHELF" class="active_input" />
     <input type="hidden" name="url_ref" value="<?php echo $_SERVER['REQUEST_URI'];?>" />
     <input type="hidden" name="book_id" value="<?php echo $_GET['id'];?>" />  
    </form>
<script type="text/javascript">
   $('form#direct_add_book input[type="button"]').click(function(){
      var data = $('form#direct_add_book input').serializeArray();
      $.post($('form#direct_add_book').attr('action'),data,function(json){
                  $('div#success_add').show(); 
             if(json){               
 }
             if(json.status=="fail"){
                 $('div#fail_add').show();                
...
?>
 <input type="button" value="BOOK IS IN YOUR SHELF" id="already_book_added" />
<?php
          }//end of check for already book added
      }//end of check for user registered
?>
    </div><!--end of add book part-->  
...
<div id="left_part">
 ...
 <div id="top_part"> 
   <div id="b_img">
    <img src="images/books/<?php echo $row2['bimage'];?>" alt="Book Image" />
     </div>    
   <div id="b_info">
     <h1 class="bname"><?php echo $row2['bname'];?></h1>
     <h1 class="aname">By: <?php echo $row2['aname'];?></h1>
 <div id="b_desc"> <p><?php echo $row2['bdesc'];?></p></div>     <!--end of b desc-->
     </div>       <!--end of b info-->
  </div> <!--end of toppart-->
 <p class="aff_link">BUY NEW FROM: &nbsp;<a href="http://flipkart.com">Flipkart</a>&nbsp;  <a href="http://amazon.in">Amazon</a> &nbsp; <a href="http://snapdeal.com">Snapdeal</a></p>
 </div>   <!--end of ab_book-->  
 <br />
 <h1 class="section_main_title">READERS WITH THIS BOOK NEAR YOU</h1>
 <?php 
         //script to show all the readers
     $query3  = "SELECT * FROM user_books WHERE book_id='".$_GET['id']."'";
     $result3 = @mysqli_query($dbc, $query3)or die("Error with query 3");
              while($row3 = mysqli_fetch_array($result3)){
     $query4  = "SELECT * FROM user_registered WHERE user_id='".$row3['user_id']."' LIMIT 1";
     $result4 = @mysqli_query($dbc, $query4)or die("Error with query4"); 
        $row4 = mysqli_fetch_array($result4);
?>
  <div class="user_tile">
  <a href="reader.php?id=<?php echo $row4['user_id'];?>">
   <div class="user_img_small"> 
     <img src="images/website-part/<?php echo $row4['image'];?>" alt="<?php echo $row4['name'];?>" />
     </div> <!--end of user image--> </a>
   <div class="right_user_part">
  <div id="send_request">
    <?php 
          //script to check for user login 
     if(!isset($_SESSION['user_id'])){
?>
   <input type="button" id="open_login_request_send" value="ASK FOR BOOK" class="active_input" />
<?php
        }else{ 
     //check for already request sent
   $query5  = "SELECT * FROM send_book_request WHERE from_user='".$_SESSION['user_id']."' AND to_user='".$row4['user_id']."' AND book_id='".$_GET['id']."'";
   $result5 = @mysqli_query($dbc, $query5)or die("Error with query5");
       if(mysqli_num_rows($result5)==0){ 
    if($_SESSION['user_id'] == $row4['user_id']){}else{
?> 
<div id="sucess_request">
 <img src="images/website-part/<?php echo $row4['image'];?>" alt="<?php echo $row4['name'];?>" /> 
 <h4>Request sent to <br /><?php echo $row4['name'];?> successfully!</h4>
</div>             <!--end of send request-->
<div id="fail_request">
 <h4>Request Failed!</h4>
</div>             <!--end of send request-->
...
setInterval(hide_msg,10000);
       function hide_msg(){
     $('div#sucess_request').fadeOut();
};  
setInterval(hidefail_msg,10000);
       function hidefail_msg(){
     $('div#fail_request').fadeOut();
}; 
</script>
    <form method="POST" action="sendbookrequest.php" id="book_request">
     <input type="hidden" name="book_id" value="<?php echo $_GET['id'];?>" /> 
     <input type="hidden" name="to_user_id" value="<?php echo $row4['user_id'];?>" /> 
     <input type="button" value="ASK FOR BOOK" class="active_input" />  
    </form>
<script type="text/javascript">
   $('form#book_request input[type="button"]').click(function(){
      var data = $('form#book_request input').serializeArray();
      $.post($('form#book_request').attr('action'),data,function(json){
         $('div#sucess_request').fadeIn();   
             if(json){             
 }
             if(json.status=="fail"){
                 $('div#fail_add_book').show();                
 }
});
}); 
</script> 
<?php
         }//end of check foe not same user
         }else{
?>
  <input type="button" id="already_req_sent" value="REQUEST SENT" />
<?php
   }//end of check for request already sent    
 }//end of check for user logged in
?>
     </div>        <!--end of send request-->
     <a href="reader.php?id=<?php echo $row4['user_id'];?>">  <h2><?php echo $row4['name'];?></h2></a>
 <?php
          if(!isset($_SESSION['user_id'])){
?>
     <h3><?php echo $row4['address'];?></h2>
<?php
             }else{
           if($row1['college']==$row4['college']){
?>
     <h3><?php echo $row4['college'];?></h2>
<?php
      }else{
?>
     <h3><?php echo $row4['address'];?></h3>
<?php
}//end of check for same college
  }//end of check for user id
?>
      </div>     <!--end of right user part-->
    </div>      <!--end of user tile-->
<?php
