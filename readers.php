...
<head>
  ...
  <title>Readers | BookBoT, Stay connected with readers near you!</title>
...
<div id="left_menu">
<div class="dashboard">
  <p>READERS IN <?php  if(isset($_GET['t'])){ echo $_GET['t'];}else{ ?>
<?php } ?></p>
...
?>
<div class="l_tile"><h1><a href="readers.php?t=COLLEGE"> COLLEGE</a></h1></div> <!--end of genre open-->
<?php
       }//end of check for user logged in
?>
<div class="l_tile"><h1><a href="readers.php?t=LOCALITY"> LOCALITY</a></h1></div> <!--end of genre open-->
<div class="l_tile"><h1><a href="readers.php"> READERS</a></h1></div> <!--end of genre open-->
</div>       <!--end of all cats-->
</div>          <!--end of left menu-->
<p id="path_all_pages">
 <a href="index.php">HOME</a> > <a href="<?php echo $_SERVER['REQUEST_URI'];?>"><?php if(isset($_GET['c'])){echo  $_GET['c'];}else{ ?> READERS  <?php }?></a>
</p>           <!--end of path all pages-->
 <div id="section_book">
 <h1 class="section_main_title">
 <?php if(!isset($_GET['t'])){
?>
       SHOWING ALL THE READERS. 
<?php
 }else{
        if($_GET['t']=='COLLEGE'){
?>
SHOWING ALL THE READERS FROM YOUR COLLEGE (<?php echo $row1['college'];?>).
<?php  
     }else{
?>
   SHOWING ALL THE READERS FROM YOUR LOCALITY.
<?php  
       }
}?>
</h1>
...
<div class="opps_n_rlt_f">
 ...
 <h1>OOPs, Sorry no reader found in your Locality!</h1>
</div>                        <!--end of opps sory no result found-->
<?php
         }//end of check for books in locality
                if($_GET['t']=='COLLEGE'){
            $query2  = "SELECT * FROM user_registered WHERE college='".$row1['college']."'";
            $result2 = @mysqli_query($dbc, $query2)or die("Error with query2"); 
                  if(mysqli_num_rows($result2)==0){
?>
<div class="opps_n_rlt_f">
 <img src="images/website-part/books.png" alt="No result found image" />
 <h1>OOPs, Sorry no reader found!</h1>
</div>                        <!--end of opps sory no result found-->
<?php            
            }else{ 
    while($row2 = mysqli_fetch_array($result2)){
?>
<div class="tile reader_tile">
 <div class="eb_img reader_tile" >
 <a href="reader.php?id=<?php echo $row2['user_id'];?>"><img rel="tooltip" title="<?php echo $row2['name'];?>" src="images/website-part/<?php echo $row2['image'];?>" alt="<?php echo $row2['name'];?>" /></a>
 </div>                                 <!--end of img div-->
 <a href="reader.php?id=<?php echo $row2['user_id'];?>" id="partial_name_show"><p class="title"><?php echo $row2['name'];?></p></a>
 <p class="view_book"><a href="reader.php?id=<?php echo $row2['user_id'];?>"></i>View Reader</a></p>
</div>            <!--end of tile--> 
<?php
                  }//end of while loop
  }//end of check for user present in college
}//end of sort by college
                                 }else{
         //showing all the readers
            $query2  = "SELECT * FROM user_registered ORDER BY rand()";
            $result2 = @mysqli_query($dbc, $query2)or die("Error with query2"); 
                  if(mysqli_num_rows($result2)==0){
?>
<div class="opps_n_rlt_f">
