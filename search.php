...
<head>
  ...
  <title><?php echo $_GET['q'];?> | BookBoT, Stay connected with readers near you!</title>
...
<div id="custom-search-input">
                            <div class="input-group col-md-12">
                                <input type="text" class="search-query form-control" id="q_search" onkeydown="searc_sug()" name="q" value="<?php echo $_GET['q'];?>" />
...
?>
<div class="l_tile"><h1><a href="search.php?t=COLLEGE&q=<?php echo $_GET['q'];?>"> COLLEGE</a></h1></div> <!--end of genre open-->
<?php
       }//end of check for user logged in
?>
<div class="l_tile"><h1><a href="search.php?t=LOCALITY&q=<?php echo $_GET['q'];?>"> LOCALITY</a></h1></div> <!--end of genre open-->
<?php
           }else{
?>
<?php 
...
<p id="path_all_pages">
 <a href="index">HOME</a> > <a href="<?php echo $_SERVER['REQUEST_URI'];?>"><?php if(isset($_GET['c'])){echo  $_GET['c'];}else{ ?> SEARCH  <?php }?></a>
</p>           <!--end of path all pages-->
 <div id="section_book">
 <h1 class="section_main_title">
   SEARCH RESULT FOR: <u><?php echo $_GET['q'];?></u>
</h1>
<?php
       if(!isset($_GET['t'])){ 
?>
<?php 
             //script to select ebooks
              ...
              $search_query.="  WHERE $where_clause ORDER BY rand() LIMIT 20";
              $search_result = @mysqli_query($dbc, $search_query)or die("Search Error");
                      }//end of where clause
...
<?php
                 ...
                 while($row10 = mysqli_fetch_array($search_result)){
?>   
<div class="tile">
 <div class="eb_img" >
    ...
    while($row2 = mysqli_fetch_array($result2)){
         if($_SESSION['user_id']!=$row2['user_id']){  
           ...
           $result3 = @mysqli_query($dbc, $query3)or die("Error with query3"); 
             ...
             //script to select books
            $query10  = "SELECT * FROM book_stock WHERE book_id='".$row3['book_id']."' AND ";
              $search_words = explode(' ', $_GET['q']);
              foreach($search_words as $word){
              $where_list[]="bname LIKE'%$word%'";
                }//end of foreach       
              $where_clause = implode(' OR ', $where_list);
              if(!empty($where_clause)){
              $query10.=" $where_clause LIMIT 20";
              $search_result = @mysqli_query($dbc, $query10)or die("Search Error");
                      }//end of where clause
                       if(mysqli_num_rows($search_result)=='0'){
?>
<div class="opps_n_rlt_f">
 <img src="images/website-part/books.png" alt="No result found image" />
 <h1>OOPs, Sorry no book found in your college!</h1>
</div>                        <!--end of opps sory no result found-->
<?php
                        }else{
                 while($row10 = mysqli_fetch_array($search_result)){
